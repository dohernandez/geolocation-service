...
golangci-lint run -c "$this_path"/../lint/.golangci-pkg.yml ./pkg/... ./internal/... ./cmd/... ./features/... || exit 1

if [[ -d "./internal" && -d "./cmd" ]]; then
    echo ">> checking unused exported symbols in internal packages"
    golangci-lint run -c "$this_path"/../lint/.golangci-internal.yml ./internal/... ./cmd/... || exit 1
fi
